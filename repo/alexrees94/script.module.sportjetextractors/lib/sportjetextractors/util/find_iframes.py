...
from .. import scanners, extractors
...
def find_iframes(url, links = [], checked = []):
    try:
        links = links
        checked = checked
        r = requests.get(url, allow_redirects=True, timeout=7, headers={"Referer": url, "User-Agent": user_agent})
            ...
            for u in urls:
                ...
                if not isAd(u) and u not in checked and __checkUrl(u) and u not in links and len(links)<15:
                    ...
                    for module in extractors.__all__:
                        if domain in module.domain or ".m3u8" in u:
...
def __checkUrl(url):
		blacklist = ['chatango', 'adserv', 'live_chat', 'ad4', 'cloudfront', 'image/svg', 'getbanner.php','/ads', 'ads.', 'adskeeper', '.js', '.jpg', '.png', '/adself.', 'min.js', 'mail.ru', "/http"]
