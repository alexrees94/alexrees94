...
class TraktLikedListManagerXML(BaseDialog):
		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					trakt_id = chosen_listitem.getProperty('venom.trakt_id')
					if chosen_listitem.getProperty('venom.isSelected') == 'true':
						...
						list_owner = chosen_listitem.getProperty('venom.list_owner')
						list_name = chosen_listitem.getProperty('venom.list_name')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('venom.list_owner', item.get('list_owner'))
					listitem.setProperty('venom.list_name', str(item.get('list_name')))
					listitem.setProperty('venom.trakt_id', str(item.get('trakt_id')))
					listitem.setProperty('venom.item_count', str(item.get('item_count')))
					listitem.setProperty('venom.likes', str(item.get('likes')))
	...
	def set_properties(self):
		try:
			...
			self.setProperty('venom.trakt_icon', joinPath(artPath(), 'trakt.png'))
		except:
			from resources.lib.modules import log_utils
			log_utils.error()