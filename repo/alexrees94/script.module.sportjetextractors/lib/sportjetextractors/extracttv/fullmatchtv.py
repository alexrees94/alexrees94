...
domain = ["fullmatchtv.com"]
site_name = "Full Match TV"
short_id = "FMTV"
...
def get_games():
    games = []
    r = requests.get(
        f"https://{domain[0]}/motorsports/",
        headers={
            "Content-Type": "application/x-www-form-urlencoded",
            "Referer": f"https://{domain[0]}/",
            "Origin": f"https://{domain[0]}",
    ...
    for league, code in leagues.items():
        ...
        response = requests.post( 
            f"https://{domain[0]}/wp-admin/admin-ajax.php",
            ...
            headers={
            "Content-Type": "application/x-www-form-urlencoded",
            "Referer": f"https://{domain[0]}/motorsports/",
            "Origin": f"https://{domain[0]}",
            "User-Agent": user_agent
        },
            params={
...
def get_m3u8(url):
    r_game = requests.get(url, headers={"Referer": domain[0], "User-Agent": user_agent}).text
    ...
    if "//ok.ru/videoembed" in r_game:
        ...
        r_embed = requests.get(embed_url, headers={"User-Agent": user_agent}).text
        try: 
            import html
            embed_json = json.loads(html.unescape(re.compile(r'data-options="(.+?)"').findall(r_embed)[0]))
        except: # Python 2
            from HTMLParser import HTMLParser
            embed_json = json.loads(HTMLParser().unescape(re.compile(r'data-options="(.+?)"').findall(r_embed)[0]))
