...
domain = ["nflstream.io", "nbastream.nu", "nhlstream.nu", "mlbstream.me", "tennisstreams.me", "rugbystreams.me", "socceronline.me", "boxingstreams.me", "ufcstream.me", "f1stream.me", "motogpstream.me", "cricstream.me"]
site_name = "NFLStream"
short_id = "NS"
        ...
        try:
            ...
            for game in soup_games.find_all("a", {"class": "btn-secondary"}):
                try:
                    ...
                    game_time = datetime(*(time.strptime(game.find_all("span")[-1].get("content"), "%Y-%m-%dT%H:%M")[:6])) - timedelta(hours=1) if game.find_all("span")[-1].get("content") != None else ""
                    games.append({
                        "title": title,
                        "links": [game_href],
