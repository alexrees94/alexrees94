...
class SportTVGuide(Extractor):
    ...
    def get_games(self):
        ...
        for m3u8_record in m3u8_records:
            ...
            if "SERVER" in fields and "://" in fields["SERVER"]: urls.append(Link(fields["SERVER"], is_ffmpegdirect=True))
        ...
        s.post("https://sport-tv-guide.live/ajaxdata/selectdate", data="d=" + datetime.datetime.now().strftime("%Y-%m-%d"), headers={"content-type": "application/x-www-form-urlencoded"})

        # Get games
        i = 0
        r = s.get("https://sport-tv-guide.live")
        page = int(re.findall(r"var page=(.+?);", r.text)[0]) + 15
        r = {"last": False}
        missing_channels = []
        while not r["last"]:
            ...
            for game in soup.select("div.row"):
                ...
                game_utc = datetime.datetime.now().replace(hour=int(game_time[0]), minute=int(game_time[1]), second=0, microsecond=0) - datetime.timedelta(hours=int(dict(s.cookies)["user_time_zone_id"]))
