...
def get_m3u8(url):
    ...
    channel = list(filter(lambda x: x["channel_id"] == channel_id, lntv.json_config["live_channels"]))[0]
    stream = channel["streams"][0]
    resolved_stream = lntv.resolve_stream(stream)
    return f"{resolved_stream[0]}|{urlencode(resolved_stream[1])}"
